	...
	return (
		<List.Item key={todo.id} className={classes.list}>
			<Group noWrap position='apart'>
				<Group>
					<ActionIcon variant='light' color='violet' size={24} radius='xl' onClick={() => updateTask(id)}>
						{isCompleted ? <CheckCircle size={18} /> : <Circle size={18} />}
					</ActionIcon>
					<Text strikethrough={isCompleted} color={isCompleted && 'dimmed'}>
						{description}
					</Text>
				</Group>
				<ActionIcon variant='light' color='violet' onClick={() => deleteTask(id)}>
					<XCircle size={18} />
				</ActionIcon>
			</Group>
		</List.Item>
